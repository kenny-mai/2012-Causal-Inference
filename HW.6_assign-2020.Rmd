...
# Generating y1s, logic for t.income means that if income was below 20, (now it's anything less than 2 because of the uniform distribution in Question 1), they weren't eligible, so their potential outcome is still linear, with treatment effect of 4. Otherwise, it becomes a new quadratic relationship. Kept tweaking the coefficient for the quadratic until the range fell between 0 and 100
fullB$y1 <- ifelse(t.income < 2,fullB$y0 + rnorm(1,4,1),0.07*t.income^2 + rnorm(1,15,1))
summary(fullB$y1)
